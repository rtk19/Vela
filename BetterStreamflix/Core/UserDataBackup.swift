...
struct UserDataBackupService {
    ...
    func restore(from data: Data) throws {
        ...
        let staging = parent.appending(
            path: ".BetterStreamflix-import-\(UUID().uuidString)",
            directoryHint: .isDirectory
        )
        let rollback = parent.appending(
            path: ".BetterStreamflix-rollback-\(UUID().uuidString)",
            directoryHint: .isDirectory
        )
...
enum UserDataBackupError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case .invalidFile:
            "This is not a valid BetterStreamflix backup."
        case let .unsupportedVersion(version):
            "This backup uses unsupported format version \(version). Update BetterStreamflix and try again."
        }
    }
}
